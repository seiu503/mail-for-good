...
const CreateCampaignForm = props => {
  ...
  const applyForm = applyTemplateValue => {
    const foundTemplate = props.templates.find(
      x => x.name === applyTemplateValue
    );
        ...
        <div>
          <label>
            <Field
              ...
              type="checkbox"
            />{" "}
            Track link clickthroughs, syntax:{" "}
            {`{linklabel/http://mylinktotrack.com}`}. Available for HTML emails
            only.{" "}
          </label>
        </div>
        ...
        <div>
          <label>
            ...
            <Field
              ...
              component="input"
              type="checkbox"
              onClick={scheduleCampaign.bind(this)}
            />{" "}
            Schedule campaign
          </label>
        </div>
...
const validate = (values, props) => {
  ...
  if (!values.listName) {
    ...
    _.find(props.lists, list => list.name == values.listName).status != "ready"
  ) {
  ...
  if (!values.emailBodyPlaintext && values.type === "Plaintext") {
    ...
    if (
      values.emailBodyPlaintext &&
      values.emailBodyPlaintext.indexOf("%%unsubscribe%%") == -1
    ) {
  ...
  if (!values.emailBodyHTML && values.type === "HTML") {
    ...
    if (
      values.emailBodyHTML &&
      values.emailBodyHTML.indexOf("%%unsubscribe%%") == -1
    ) {
  ...
  if (!values.emailBodyHTMLEditor && values.type === "HTMLEditor") {
    ...
    if (
      values.emailBodyHTMLEditor &&
      values.emailBodyHTMLEditor.indexOf("%%unsubscribe%%") == -1
    ) {
