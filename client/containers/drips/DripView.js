...
const mapDispatchToProps = { initialize, notify, getLists, getTemplates, getDrips };
...
export class DripViewComponent extends Component {
    ...
    getSingleDrip(props) {
        ...
        if (slug) {
            ...
            let listName = this.props.lists.find(lists => lists.id === getDripBySlug.listId);
            
            let sequences = JSON.parse(getDripBySlug.sequences);
            let previewSequences = [];
            Object.keys(sequences).forEach(key => {
                let templateName = this.props.templates.find(template => template.id === sequences[key].templateId);
                previewSequences[previewSequences.length] = { sequenceday: sequences[key].sequenceday, templateName: templateName.name };
            });
