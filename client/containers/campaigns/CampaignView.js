...
export class CampaignViewComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const justSentCampaign =
      !nextProps.isPostingSendCampaign && this.props.isPostingSendCampaign;
    ...
    const justSentTestEmail =
      !nextProps.isPostingSendTest && this.props.isPostingSendTest;
  ...
  getSingleCampaign(props) {
    ...
    const getCampaignBySlug = props.campaigns.find(
  ...
  render() {
    ...
    const downloadUnsentSubscribersUrl = encodeURI(
      `${window.location
        .origin}/api/campaign/subscribers/csv?campaignId=${thisCampaign.id}&sent=false`
    );
            ...
            <div className="box-body">
              {sendCampaignResponse &&
                haveShownMessage && (
                  <p
                    className={
                      sendCampaignStatus === 200 ? "text-green" : "text-red"
                    }
                  >
                    <i
                      className={
                        sendCampaignStatus === 200
                          ? "fa fa-check"
                          : "fa fa-exclamation"
                      }
                    />{" "}
                    {sendCampaignResponse.split(".")[0]}.<br /> <br />{" "}
                    {sendCampaignResponse.split(".")[1]}.
                  </p>
                )}
