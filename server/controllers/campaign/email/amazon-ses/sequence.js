...
const getArrayOfCampaignSequences = require("./controllers/getArrayOfCampaignSequences");
const getAmazonEmailArrayForSequence = require("./controllers/getAmazonEmailArrayForSequence");
...
module.exports = async function(
  ...
  const notificationSentEmails = sendUpdateEmailsSentNotification(
    campaignInfo,
    io,
    req
  );
  ...
  const campaignSubscriberStatus = true;
  const arrayOfIds = nestArray(
    NESTED_ARRAY_LENGTH,
    await getArrayOfEmailIds(campaignInfo, campaignSubscriberStatus)
  );
  ...
  for (let i = 0; i < LENGTH_OF_LIST_SUBSCRIBER_IDS; i++) {
    ...
    let amazonEmailArray = await getAmazonEmailArrayForSequence(
      currentBlockOfEmails,
      campaignInfo,
      whiteLabelUrl,
      CampaignSequences
    );
