  ...
  Promise.all([campaignBelongsToUser, getAmazonKeysAndRegion])
    .then(() => {
      ...
      if (campaign.trackingPixelEnabled) {
        campaign.emailBody = insertTrackingPixel(
          campaign.emailBody,
          "example-tracking-id",
          campaign.type
        );
      ...
      db.list
        ...
        .then(list => {
          ...
          const additionalData = list.additionalFields.reduce(
          ...
          campaign.emailBody = mailMerge(
            { email: testEmail, additionalData },
            campaign
          );
